...
export default function KeyManagement() {
      ...
      onConnect: () => {
        ...
        client.subscribe('/topic/key-distribution', (msg) => {
          ...
          if (data.status === 'READY') {
            ...
            keyManagementApi.getActiveRecovery()
              .then(resRec => {
                if (resRec && resRec.shares) {
                  setUploadedShares(resRec.shares)
                }
              })
  ...
  const handleRestoreFilesChange = async (e) => {
    if (e.target.files && e.target.files.length > 0) {
      try {
        ...
        // file lists are fully synchronized in real-time via WebSocket/shares
        setRestoreFiles([])
      } catch (err) {
  ...
  const handleRevokeShare = async (x) => {
    try {
      setActionLoading(`revoke_${x}`)
      setStatusMessage(null)
      await keyManagementApi.deleteRecoveryShare(x)
      setStatusMessage({ type: 'success', text: 'Thu hồi mảnh khóa thành công!' })
    } catch (err) {
      setStatusMessage({ type: 'error', text: 'Thu hồi mảnh khóa thất bại: ' + (err.response?.data || err.message) })
    } finally {
      setActionLoading(null)
    }
  }
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 rounded-3xl shadow-sm">
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className={`w-full flex items-center gap-2 font-bold py-3 px-4 rounded-xl transition-all text-sm ${activeTab === 'split'
              ...
              <button
                ...
                className={`w-full flex items-center gap-2 font-bold py-3 px-4 rounded-xl transition-all text-sm ${activeTab === 'restore'
                    ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30 shadow-sm'
                    : 'bg-slate-50 text-slate-600 dark:bg-slate-800 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 border border-transparent'
                  }`}
              >
                    ...
                    {uploadedShares.length > 0 && (
                      <div className="space-y-3">
                        <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                          Danh sách mảnh khóa đã tải lên ({uploadedShares.length})
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {uploadedShares.map((share) => {
                            const isCurrentUser = user?.username === share.uploadedBy;
                            const isAdmin = user?.role === 'ADMIN';
                            const canDelete = isAdmin || isCurrentUser;
                            const isDeleting = actionLoading === `revoke_${share.x}`;

                            return (
                              <div key={share.x} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-850 hover:border-indigo-100 dark:hover:border-indigo-500/20 transition-all shadow-sm">
                                <div className="flex items-center gap-3">
                                  <div className="p-2.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-xl">
                                    <Key size={18} />
                                  </div>
                                  <div className="min-w-0">
                                    <p className="text-sm font-bold text-slate-800 dark:text-slate-200 truncate max-w-[180px]" title={share.fileName}>
                                      {share.fileName}
                                    </p>
                                    <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                                      Tải lên bởi: <span className="font-semibold text-slate-600 dark:text-slate-400">{isCurrentUser ? 'Tôi' : share.uploadedBy}</span> • {new Date(share.uploadedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                    </p>
                                  </div>
                                </div>
                                ...
                                {canDelete && (
                                  <button
                                    onClick={() => handleRevokeShare(share.x)}
                                    disabled={actionLoading !== null}
                                    className="p-2 rounded-xl text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-600 dark:hover:text-red-400 transition-all disabled:opacity-50"
                                    title="Thu hồi file này"
                                  >
                                    {isDeleting ? (
